...
def is_supported(context, **kwargs):
    try:
        assert int(context.moon_phase_index) in (24, 25, 26, 27)
...
def find_main_chord(scale, aeolian_harp, minp=3, maxp=4):
    ...
    chord_tuple = project_generators.find_chord_tuple(
        ...
        pitch_count_range=ranges.Range(minp, maxp),
...
def make_sequential_event(string, level, duration, random):
    ...
    match level:
        ...
        case 9:
            tone_duration_range = ranges.Range(4, 10)
        case _:
