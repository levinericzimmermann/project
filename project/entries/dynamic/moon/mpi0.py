...
def is_supported(context, **kwargs):
    try:
        assert int(context.moon_phase_index) in (1,)
...
def main(
    ...
    assert len(simultaneous_event) == 9
...
def make_sequential_event(string, duration, level, activity_level, random):
    duration_range_tuple = (ranges.Range(180, 221), ranges.Range(500, 940))
    ...
    while seqd < duration:
        ...
        if is_active := activity_level(level):
            ...
            note_like.frequency_factor = random.uniform(0.055, 0.1)
