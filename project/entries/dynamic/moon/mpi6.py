...
def is_supported(context, **kwargs):
    try:
        assert int(context.moon_phase_index) in (6, 7, 8)
    ...
    for string in aeolian_harp.string_tuple:
        ...
        simultaneous_event.append(
            make_sequential_event(
                string, duration, random.choice((6, 7, 8)), activity_level, random
            )
        )
...
def make_sequential_event(string, duration, level, activity_level, random):
    duration_range_tuple = (ranges.Range(6, 15), ranges.Range(1.2, 4))
    ...
    while seqd < duration:
        ...
        if is_active := activity_level(level):
            ...
            note_like.frequency_factor = random.uniform(0.015, 0.02)
